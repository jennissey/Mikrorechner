...
architecture testbench of procTst is
  ...
  signal Data, memData, IR, memAdress, PCout : signed(31 downto 0);
  signal dBus : std_logic_vector (31 downto 0);

component Pipeline is
port	( clk, nRes		: in	std_logic;
	  IR, DATA		: in	signed(31 downto 0);
	  memAdress, memData, PCout: out	signed(31 downto 0);
...
begin
  ...
  instMemI: sram	generic map (	addrWd	=> 8,
			...
			port map    (	nCS	=> const0,
					nWE	=> const1,
					nOE	=> const0,
					addr	=> std_logic_vector(PCout(7 downto 0)),
					data	=> std_logic_vector(IR),
					fileIO	=> iCtrl);
  dataMemI: sram	generic map (	addrWd	=> 8,
			...
			port map    (	nCS	=> const0,
					nWE	=> dnWE,
					nOE	=> dnOE,
					addr	=> std_logic_vector(memAdress(7 downto 0)),
					data	=> dBus,
					fileIO	=> dCtrl);
dBus <= DATA when dnWE = '1' else (others => 'Z');
